...
from app.auth import require_admin_auth, get_current_admin
from app import db
...
@require_admin_auth
def list_all_urls():
    """List all URLs with pagination and sorting (admin only)"""
    try:
...
def list_users():
    ...
    try:
        ...
        query = db.session.query(
            User,
            db.func.count(URL.id).label('url_count')
        ).outerjoin(URL).group_by(User.id).order_by(User.created_at.desc())
